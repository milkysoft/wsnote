...
module RemoteBlogData = 
    ...
    type Message = 
        ...
        | GetPage of int * int * string * AsyncReplyChannel< int * Post list >
        ...
        and loop blog = async {
            ...
            | GetPage (pageNum, postsCountOnPage, context, replyChannel) ->
                let xs = 
                    blog
                    |> Map.toList
                    |> List.mapi( fun n (id,(cd, ed, ttl, cntnt)) -> n, id, cd, ed, ttl, cntnt )
                    |> List.filter( fun (_,_,_,_, ToLower title, ToLower content) -> 
                        match context with
                        | "" -> true
                        | ToLower context ->   
                            title.Contains(context) || content.Contains (context) )
                    |> List.sortBy ( fun (_,_,dt,_,_,_) -> DateTime.MaxValue - dt)
                let total'count = xs.Length
                let xs = 
                    xs
                    |> List.mapi( fun n (num, id, cd, ed, ttl, cntnt) -> 
                        n,  {   Id = id
                                Num = num
                                Title = ttl
                                Content = cntnt
                                CreateDate = s'dttm cd
                                EditDate = s'dttm ed  } )
                    |> List.filter( function (n,_) -> n >= pageNum * postsCountOnPage && n < (pageNum + 1) * postsCountOnPage )                
                    |> List.map snd
                replyChannel.Reply(total'count,xs)
                return! loop blog 
